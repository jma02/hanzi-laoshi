...
The interface uses contemporary Chinese typography and natural Chinese labels, with English explanations below the main learning instructions. Navigation, feedback, topics, and progress use Chinese-first hierarchy. The four-line pinyin bands and large Kai-style practice characters remain focused on reading, while warm cream surfaces and small tea and sprout sprites keep the atmosphere friendly.

Fonts are self-hosted under `static/fonts/` with their OFL licenses. `hanzi-ui.woff2` is a renamed, variable 400–700 subset of [Noto Sans SC](https://github.com/google/fonts/tree/main/ofl/notosanssc), used for the interface. To update its coverage after changing interface text, run `python scripts/subset-ui-font.py /path/to/NotoSansSC.ttf` with `fonttools[woff]` installed; the script includes source text, pinyin, and the existing corpus's traditional characters. `hanzi-classroom.woff` is a renamed [LXGW WenKai](https://github.com/lxgw/LxgwWenKai) subset for practice characters; unsupported glyphs fall back to system Kai/Song fonts. `hanzi-text-*.woff` are Latin [Alegreya Sans](https://github.com/huertatipografica/Alegreya-Sans) subsets for pinyin, including every tone mark.

The classroom illustration was generated for this project. `static/cozy-sprites.webp` was generated with the built-in imagegen tool from this prompt: “A transparent two-cell pixel-art sprite sheet: a smiling cream teacup with terracotta rim and steam, and a smiling terracotta pot with a two-leaf sprout; warm cream, terracotta, sage, honey, and brown; no text or scene.” It is one optimized 512 × 256 WebP. `CozySprite.svelte` selects each half with CSS and hides these decorative images from assistive technology.
